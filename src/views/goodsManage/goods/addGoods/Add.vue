...
import { getCategories, getParams,addGoods } from "@/api/goods";
export default {
  data() {
    return {
      activeIndex: "0",
  ...
  methods: {
    ...
    async tabClicked() {
      ...
      if (this.activeIndex === "1") {
        const res = await getParams(this.cateId, {
    ...
    add() {
      this.$refs.addFormRef.validate(async(valid) => {
        ...
        // 发起请求添加商品
        // 商品的名称，必须是唯一的
        const res = await addGoods("goods", form);
